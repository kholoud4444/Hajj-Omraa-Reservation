...
const Addapp = ({ OnAddAppointment }) => {
    ...
    const [omraname, setOmraname] = useState("");
    ...
    const submitHandler = async (e) => {
        ...
        try {
            const response = await axios.post(
                'http://localhost:5001/api/omra',
                { omraname, from_where, to_where, ticket_price, day_and_time, max_num_trav }
            );
    ...
    return (
        ...
        >
            <h4>Add Umrah Appointment</h4>
            <div>
                <label>Umrah Appointment Name</label>
                <input
                    type='text'
                    className='form-control'
                    required
                    value={omraname}
                    onChange={(e) => setOmraname(e.target.value)}
                />
            </div>
